...
# Parse into example single trial movies // THis is just the moviedata
...
fns = sorted(glob.glob('/data/fUS_project/data/data_july05/RT*.mat'))
timelines = len(fns)*['/data/fUS_project/data/data_july05/timeline_07-05-2019_13-37.mat']
n_fus = range(len(fns))
...
for ii in range(len(all_exps)-1):
    ...
    data_retin = data_resample_at_stim[:480, :, :]
    data_spots = data_resample_at_stim[-300:, :, :]
    ...
    for i in range(n_trials):
        for k in range(3):
            stim[k, i*30 + 10*k : i*30 + 10*k + 10] = 1;
    #plt.plot(stim.T)
    ...
    data_retin_list.append(data_retin)
