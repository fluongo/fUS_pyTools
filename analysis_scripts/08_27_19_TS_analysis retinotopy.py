...
# Load the fUS data
##################################################################
fns = sorted(glob.glob('/data/fUS_project/data/data_aug27/RT*.mat')); # Only first 7 are the retinotopu
timelines = len(fns)*['/data/fUS_project/data/data_aug27/timeline_08-27-2019_11-02.mat']
...
# list_suby   = [(yy[idx, xi, yi] - np.median(yy[idx, xi, yi])) / np.median(yy[idx, xi, yi]) for yi in range(nY)]
# list_xvals  = [list(np.arange(1200)) for i in range(nY)]
# results = Parallel(n_jobs=20)(delayed(compute_ts_smooth_kde)(i,j,k) for i,j,k in zip(list_subx,list_suby, list_xvals))
