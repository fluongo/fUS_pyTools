...
#%%
fns = sorted(glob.glob('/data/fUS_project/data/data_July12/RT*.mat'))
fns = fns[:6]
timelines = len(fns)*['/data/fUS_project/data/data_July12/timeline_07-12-2019_13-16.mat']
n_fus = range(len(fns))
...
for ii in range(len(all_exps)):
    ...
    if ii in [0]:
        ...
        #for i in range(6): print(len(fus_list[i]))
...
#%% Try a kernel density estimate to remove outliers
def compute_ksdensity(data):
    ''' Computes a ksdensity estimate of the t, y , x''' 
...
for i in range(25):
    plt.subplot(5,5,i+1)
    plt.imshow(np.abs(out[i, :, :]), cmap = 'Greys'); 
    plt.title('power'); plt.colorbar()
