...
# Load the fUS data
##################################################################
fns = sorted(glob.glob('/data/fUS_project/data/data_aug19/RT*.mat')); # Only first 7 are the retinotopu
timelines = len(fns)*['/data/fUS_project/data/data_aug19/timeline_08-19-2019_13-58.mat']
n_fus = range(len(fns))
...
for ii in range(len(all_exps)):
    ...
    trials_all = {}
    trials_all['az_LR'] = data_resample_at_stim[:1800, :, :]
    trials_all['az_RL'] = data_resample_at_stim[1800:, :, :]
...
idx = np.where((curr_x>1200*15) & (curr_x<=1200*30-2))
X = np.mod(curr_x[idx].reshape(-1, 1), 1200)
...
# MAKE X
curr_x = all_exp_dicts[exp_no]['fus_ts_stim']
idx = np.where((curr_x>1200*0) & (curr_x<=1200*15))
X = np.mod(curr_x[idx].reshape(-1, 1), 1200)
# MAKE Y
y = all_exp_dicts[exp_no]['data_raw_fix'][idx, 51, 15]
y = y/np.mean(y)
