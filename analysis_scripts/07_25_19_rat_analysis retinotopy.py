...
# Load the fUS data
##################################################################
fns = sorted(glob.glob('/data/fUS_project/data/data_July25/RT*.mat'))[:7]; # Only first 7 are the retinotopu
...
for ii in range(len(all_exps)):
    ...
    # Median filter
    data_raw = median_filter(data_raw, size = [5,3,3])
...
# Export raw
scio.export_tiffs(all_trials_raw, exportDir + 'RETINOTOPY_cumulative_trials_RAW_medfilt_533.tiff', dims = {'x':2, 'y':1, 't':0})
