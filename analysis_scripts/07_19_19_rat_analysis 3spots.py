...
timelines = len(fns)*['/data/fUS_project/data/data_July18/timeline_07-19-2019_11-35_3spot.mat']
n_fus = range(len(fns))
...
for ii in range(len(all_exps)):
    ...
    stim = np.zeros([3, 300*fus_rate])
    for i in range(n_trials):
        for k in range(3):
            stim[k, i*30*fus_rate + 10*fus_rate*k : i*30*fus_rate + 10*fus_rate*k + 5*fus_rate] = 1;
    #plt.plot(stim.T)
