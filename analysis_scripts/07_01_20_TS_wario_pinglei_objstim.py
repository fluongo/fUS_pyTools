...
# Load the fUS data
##################################################################
data_dir = '/data/fUS_project/data/data_july01_2020/'
fns = sorted(glob.glob(data_dir + '/RT*.mat')); # 2 planes
timelines = len(fns)*[data_dir + '/timeline_07-01-2020_14-50.mat']
n_fus = range(len(fns))
...
for ii in range(n_exp):
    ...
    exportDir = outDir + '/extras_pinglei_obj_firstAnalysis_frankJoe/'
    ...
    if ii == 0:
        stim_ts = np.append(stim_ts[0], stim_ts)
...
trials_tmp = trials_dff[60:, :, :]# Chop off 30sec gray
...
#block_order: {'objects_spikey'  'animals_body'  'faces'  'object_square'  
# 'N_objects_spikey'  'N_animals_body'  'N_faces'  'N_object_square'}
...
trials_dff_sub = trials_dff[60:, :, :].reshape([10, 320, nHigh*nY, nWide*nX])


# Last 5 seconds of stimulus on
intervals = [(10, 20), (50, 60), (90, 100), (130, 140), (170, 180), (210, 220), (250, 260), (290, 300)]
...
# Compute one vs all for each of the 4 categories
...
for i,s in zip(range(4), ['skinny', 'bodies', 'faces', 'stubby']):
    plt.subplot(2,2,i+1)
    plt.imshow(p_sign[:, :, i]*np.log10(p_vals[:, :, i]), cmap = 'PuRd_r', vmin = -3, vmax = -1.3); 
