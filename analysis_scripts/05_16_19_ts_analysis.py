...
mov_fn = '/data/linuxData/MASTER_STIMULUS_FOLDER/STIMULUS_GENERATION_HERE/Francisco/fUS_3point_retinotopy/fUS_retinotopy_3point_10repeats_blackBG.mat'
...
fns = sorted(glob.glob('/data/fUS_project/data/data_may16/RT*.mat'))
timelines = len(fns)*['/data/fUS_project/data/data_may16/timeline_05-16-2019_11-33.mat']
n_fus = range(len(fns))
...
for ii in range(len(all_exps)):
    ...
    data_resampled_at_stim_list.append(data_resample_at_stim)
...
nF_stim = data_resample_at_stim.shape[0]
...
for i in range(len(all_exps)):
    ...
    if not do_resample:
        ...
        all_trials_resampled_at_stim[:, a*nY:(a+1)*nY, b*nX:(b+1)*nX] = data_resampled_at_stim_list[i]
