...
animal_fn, timeline_fn, exp = all_exps[3]
...
reload(fUS)
ph_ev, pw_ev = fUS.compute_fft(trials_el.mean(axis=1), dims = [1, 2,0])
ph_az, pw_az = fUS.compute_fft(trials_az.mean(axis=1), dims = [1, 2,0])
fs = fUS.compute_field_sign(ph_ev, ph_az)
